            ...
            async (message): Promise<MessageView | null> => {
                ...
                if (message.attachments.size > 0) {
                        ...
                        if (a.contentType?.startsWith("audio/")) {
                            try {
                                ...
                                if (response.ok) {
                                    ...
                                    const sessionResult = await manager.loadModel(AvailableModels.WHISPER_TINY, () => {}, () => {});
                                    if (sessionResult.isOk) {
                                        const session = sessionResult.value;
                                        const transcriptionResult = await session.transcribe(new Uint8Array(audioBuffer), false, {});
                                        if (transcriptionResult.isOk) {
                                            const transcription = transcriptionResult.value;
                                            return { type: "transcription", name: a.name, content: transcription.text };
                                        }
                                    }
                                }
                            } catch (error) {
                    ...
                    const processedAttachments = (await Promise.all(attachmentProcessingPromises)).filter(
                        (a): a is { type: "transcription"; name: string; content: string } | {
                            type: "media";
                    ...
                    for (const attachment of processedAttachments) {
                        if (attachment.type === "transcription") {
