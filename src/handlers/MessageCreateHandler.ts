    ...
    public async handle(message: Message): Promise<void> {
        ...
        try {
            ...
            for (const part of messageParts) {
                ...
                // If we have a character and webhook manager, and we're in a guild channel
                if (character && webhookManager && message.guild && message.channel instanceof TextChannel) {
                    ...
                    // For DMs or when no character is selected, use regular reply
                    if (message.channel.type === ChannelType.DM) {
                        // In DMs, use embeds
                        ...
                        // In guilds without a character, send plain text
                        const url = `https://discord.com/users/${message.author.id}`;
                        const link = hyperlink(`Generated by ${message.author.displayName}`, hideLinkEmbed(url));
                        await message.reply({
                            content: `${part}\n${link}`,
