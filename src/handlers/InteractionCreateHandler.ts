    ...
    private async handleComponentInteraction(interaction: Interaction, logContext: string) {
        ...
        if (interaction.customId === "delete") {
            if (interaction.channel?.type !== ChannelType.DM) {
                const authorIdMatch = (message.components as any).flatMap((row: any) => row.components).find((c: any) =>
                    "content" in c && c.content?.includes("https://a.a/")
                )?.content?.match(/https:\/\/a\.a\/(\d+)/);
