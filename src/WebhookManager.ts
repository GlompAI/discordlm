...
export class WebhookManager {
    ...
    async sendAsCharacter(
        ...
        try {
            const authorId = messageToReply ? messageToReply.author.id : null;
            const botName = this.client.user?.username || "Aria";
            const url = authorId ? `https://a.a/${authorId}` : "https://a.a";
            const sendOptions: WebhookMessageCreateOptions = {
                content: `${content}\n\n-# [Generated by ${botName}](${url})`,
                username: character.card.name,
                components: [
                    { type: 1, components: [{ type: ComponentType.Separator }] },
                    componentService.createActionRow(),
                ],
                ...options,
            };
    ...
    async editAsCharacter(
        ...
        try {
            ...
            const botName = this.client.user?.username || "Aria";
            const url = authorIdMatch ? `https://a.a/${authorIdMatch[1]}` : "https://a.a";
