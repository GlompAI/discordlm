...
export class WebhookManager {
    ...
    async sendAsCharacter(
        ...
        try {
            ...
            const footerId = authorId ?? `Generated by: ${this.client.user?.username || "Aria"}`;
    ...
    async editAsCharacter(
        ...
        try {
            const authorIdMatch = message.content.match(/\[(.*?)\]\(https:\/\/a\.a\)/);
            const originalFooterId = authorIdMatch
                ? authorIdMatch[1]
                : `Generated by: ${this.client.user?.username || "Aria"}`;
            const editOptions: WebhookMessageCreateOptions = {
                content: `${content}\n-# [${originalFooterId}](https://a.a)`,
                ...options,
            };
